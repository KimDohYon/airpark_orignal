...
def setup_inference_function(saver_path: pathlib.Path,
                             file_name: str = 'saved_model.meta') \
        -> Callable[[np.ndarray], List[float]]:
    """ Sets up a tensorflow session from a saved tf session to perform inference

    :param saver_path:  Path to meta file
    :param file_name:   meta file name
    :return:            Callable inference function
    """

    # Support recent GPU architectures that require dynamic memory sizing
    config = tf.compat.v1.ConfigProto()
    config.gpu_options.allow_growth = True

    # Define input and output node names from model
    in_node_name = 'input/IteratorGetNext:0'
    out_node_name = 'output/networkout:0'

    # Load saved model
    metafile = saver_path / file_name
    saver = tf.compat.v1.train.import_meta_graph(str(metafile))

    # Define TF graph/session and restore current model into session
    graph = tf.compat.v1.get_default_graph()
    session = tf.compat.v1.Session(config=config)
    saver.restore(session, tf.train.latest_checkpoint(saver_path))

    # Extract placeholders, nodes, and operations from trained model
    inputlayer = graph.get_tensor_by_name(in_node_name)
    networkout = graph.get_tensor_by_name(out_node_name)
    prediction = tf.nn.softmax(networkout, name='softmax')

    # Create a lambda function to perform inference using TensorFlow
    def infer_func(x):
        return session.run(prediction, feed_dict={inputlayer: x})
...
def infer(data_folder: Union[str, os.PathLike],
    ...
    """ This script will re-initialize a trained TensorFlow model for inference. It will
    ...
    tf.compat.v1.reset_default_graph()  # reset graph in case using same script as training
